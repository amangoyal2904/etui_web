          ...
          if (nm !== "Brand Solutions") {
            return (
              <React.Fragment key={`more-nav-${index}`}>
                <a href={nmSection.link ? nmSection.link : '#;'} className={styles.subsec1} rel={nmSection.rel}>{nmSection.nm}</a>
                {/* only render sub-sections if they exist */}
                {subSec && subSec?.map((subSection, index2) => {
                  return (
                    <React.Fragment key={`more-nav-${index}-${index2}`}>
                      <a href={subSection.link ? subSection.link : '#;'} className={styles.subsec2} rel={subSection.rel}>{subSection.nm}</a> 
                    </React.Fragment>
                  )
                })}
              </React.Fragment>
