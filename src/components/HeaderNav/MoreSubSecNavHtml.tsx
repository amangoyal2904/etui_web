...
const MoreSubSecNavHtml: FC<SubSecNavProps> = ({ subsecnames, data, index }) => {
  ...
  return (
    <React.Fragment key={keyName}>
      ...
      <a key={`l2_${index}`} href={data.link ? data.link : '#;'} itemProp="url"  className={`${subsec2 == data.msid && styles.current} ${styles.subsec1}`}>
        ...
        data.sec?.map((l2, index2) => {
          ...
          return (
            <React.Fragment key={keyName_l2}>
              <a href={l2.link ? l2.link : '#;'} className={`${styles.subsec2} ${subsec3 == l2.msid && styles.current}`}>
                {l2.nm}
              </a>
              {/* Check if there are any sub-sections */}
              {l2.sec 
                && l2.sec.map((l3, index3) => {
                  ...
                  return (
                    <React.Fragment key={keyName_l3}>
                      <a href={l3.link ? l3.link : '#;'} className={styles.subsec3} data-ga-onclick={l3.link}>
                        {l3.nm}
                      </a>
                    </React.Fragment>
                  );
