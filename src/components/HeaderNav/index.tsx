...
const HeaderNav: React.FC<HeaderNavProps> = ({ menuData, subsecnames, page }) => {
  ...
  return (
    <>
          ...
          {sectionList?.map((data, index) => {
            return (
              <div key={`nav-l1-${index}`} className={`sec_1 hasSubSecnav ${styles.sec_1} ${data.hovernav ? styles.hasSubSecnav : ""}`} data-l1={data.nm} data-id={data.msid}>
                <a className={`${((typeof data.msid != "undefined" && subsec1 == data.msid)  || (typeof data.msid == "undefined" && page == "subscriberhome" && data.nm == "Home")) ? styles.current : ''} ${data.msid === '74912912' ? `${styles.navP} ${styles.cSprite_b}` : ''} ${data.msid === '110737294' ? `${styles.marketData_i}` : ''}`} href={data.link ? data.link  : ''} data-ga-onclick={data.link}>
      ...
      {page == "subscriberhome" && !isWeekend && <StockTalkWidget />}
    </>
  );
};
