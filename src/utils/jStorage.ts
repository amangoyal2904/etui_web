type XMLData = {
  ...
  const jStorageReact = (() => {
    let c: StorageObject = { __jstorage_meta: { CRC32: {}, TTL: {} } };
    ...
    const E = {
      isXML: (a: any) =>
        a && (a.ownerDocument || a).documentElement ? "HTML" !== (a.ownerDocument || a).documentElement.nodeName : false,
    ...
    if (localStorageAvailable) {
      try {
        const storedData = l.getItem("jStorage") || "{}";
        c = n.parse(storedData);
        if (!c.__jstorage_meta) {
          c.__jstorage_meta = { CRC32: {}, TTL: {} }; // Ensure __jstorage_meta is initialized
        } else {
          if (!c.__jstorage_meta.CRC32) c.__jstorage_meta.CRC32 = {};
    ...
    const flush = () => {
      c = { __jstorage_meta: { CRC32: {}, TTL: {} } }; // Reset to empty state
      x();
      w();
      return true;
    };
